...
import (
	...
	mockUc "warung-pintar/item-service/mocks/app/use_cases"
	mockResp "warung-pintar/item-service/mocks/interface/rest/response"
...
func (s *HandlerTest) TestGetItemSuccess() {
	bodyBytes, _ := json.Marshal(s.dtoTest)

	r := httptest.NewRequest("POST", "/items/", bytes.NewBuffer(bodyBytes))
...
func (s *HandlerTest) TestFailDTO() {
...
func (s *HandlerTest) TestFailValidateDTO() {
...
func (s *HandlerTest) TestFailNotFound() {
	bodyBytes, _ := json.Marshal(s.dtoTest)
...
func (s *HandlerTest) TestFailRetrieve() {
	bodyBytes, _ := json.Marshal(s.dtoTest)
